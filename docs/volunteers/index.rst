...
:class:`~fyt.applications.models.Volunteer` holds shared information (contact info, medical certifications, trip assignments, etc.)
...
.. todo::
   ...
   I think it would be semantically better to have *four* models: The three we currently have plus a general ``Volunteer`` model to which we move things like application status and assignments from ``Volunteer``. This will require reworking most of the ``applications`` app, but if anyone has the time...
