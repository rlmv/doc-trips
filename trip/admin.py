...
class CampsiteAdmin(admin.ModelAdmin):
    ...
    def __getattr__(self, attr):
        ...
        making m*n queries, one for each entry in the table. Can we cache anything?
        ...
        try:
            ...
            def date_capacity_getter(campsite):
                ...
                resident_trips = ScheduledTrip.objects.filter(trips_year=trips_year).filter(
                    Q(template__campsite_1=campsite, 
                ...
                num_residents = 0
                for trip in resident_trips:
                    if date == trip.section.at_campsite_1 and campsite == trip.template.campsite_1:
                        num_residents += trip.template.max_num_people
                    if date == trip.section.at_campsite_2 and campsite == trip.template.campsite_2:
                        num_residents += trip.template.max_num_people
